...
while cap.isOpened():
    ...
    if key == ord('l') and locked_det_id is None:
        if boxes.id is None:
            cv.putText(out, "No track IDs yet — try again",
                       (20, 40), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        else:
            best_det_idx = int(boxes.conf.argmax())
            locked_det_id = int(boxes.id[best_det_idx])
            text_size = cv.getTextSize("Status: Finding BPM", cv.FONT_HERSHEY_SIMPLEX, 0.9, 2)[0]
            text_x = (out.shape[1] - text_size[0]) // 2
            cv.putText(out, "Status: Finding BPM", (text_x, 40), cv.FONT_HERSHEY_SIMPLEX, 
                       0.9, (0, 255, 255), 2)
            start_time = time.perf_counter()
